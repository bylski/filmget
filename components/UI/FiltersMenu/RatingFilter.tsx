...
const RatingFilter = () => {
  ...
  const savedRatingRange = useAppSelector(
    (state) => state.mediaFilter.ratingRange
  );
  ...
  const sliderChangeHandler = (currentRange: any) => {
    ...
    if (rangeFrom === rangeTo && rangeFrom !== 0) {
      ...
      dispatch(
        mediaFilterActions.getRatingRange({
          ratingRange: [rangeFrom - 1, rangeTo],
        })
      );
      return;
      ...
      dispatch(
        mediaFilterActions.getRatingRange({
          ratingRange: [rangeFrom, rangeTo + 1],
        })
      );
      return;
    }
